...
void plotTogetherEfficiencyCompareImages(TString fileName1, TString fileName2, TString folder, TString legend1, TString legend2) {
    ...
    TString grNames[] = {"grTpcAccHftPidPreCutsBDT", "grTpcAcc", "grTpcAccHftPid", "grTpcAccHft", "grTpcAccHftPidPreCuts", "grTpcAccHftPreCuts", "grTpcAccHftPreCutsBDT", "grAcc"};
    const int nGrNames = sizeof(grNames) / sizeof(TString);

    plot(inputPlot, 2, legendNamesPlot, grNames, nGrNames);
    TString folderOut = Form("finalAnalysis/efficiencyRatio/%s/%s_to_%s", folder.Data(), fileName1.Data(), fileName2.Data() );
...
//                                "ntp_fullEvent_full_production.vtx.3M.1308.AllD0"};
    ...
    ///toto chcem:
    TString inputPlot[] = {"ntp.FS.1cm.all.newE.1509.goodPid",
    ...
    TString legendNamesPlot[] = {"global, DCA < 1 cm",
    ...
    plot(inputPlot, 5, legendNamesPlot, grNames, nGrNames);
